...
`Analytics`, `Automations`, `Media`, `Inbox`, `Contacts`, `Ads`, `Validate`.
...
| `posts.go` `accounts.go` `workspaces.go` `communities.go` `labels.go` `webhooks.go` `analytics.go` `automations.go` `media.go` `inbox.go` `contacts.go` `ads.go` `validate.go` | one `*Service` each, with its request/response types |
